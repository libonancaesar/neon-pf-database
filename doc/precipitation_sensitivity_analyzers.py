...
def _check_percent_uncertainty(row, col_names):
    value = row.loc[col_names[0]] # precip
    uncertainty = row.loc[col_names[1]] # uncertainty
    if (uncertainty == 0) and (pd.notna(value)):
        return 0
...
def precip_flagger(data: pd.DataFrame, uncertainty_threshold: int = 0.05) -> pd.DataFrame:
    """Filter precipitation data with quality flag.

    Args:
        data (pd.DataFrame): precip data.
        uncertainty_threshold (int): threhold to filter out the measurements. 
    Raises:
    ...
    if int(resolution) == 5:
        ...
        data["flagPercent"] = data.apply(_check_percent_uncertainty, 
                                          args=([data_field, "priPrecipExpUncert"],), axis=1)
     
        # Flag the time stamp when the quality flag is raised i.e., finalQF == 1 NEON.DOC.00898, p.18, eq.(26)
        flag = (data["priPrecipFinalQF"] == 1) | (data["flagPercent"] >= uncertainty_threshold) 
        
    elif int(resolution) == 1:
        data_field = "secPrecipBulk"
        data["flagPercent"] = data.apply(_check_percent_uncertainty, 
                                         args=([data_field, "secPrecipExpUncert"],), axis=1)
        ...
        flag_uncertainty = (data["flagPercent"] >= uncertainty_threshold)
        flag = (flag_range | flag_scirvw | flag_uncertainty) 
    else:
...
def precip_event_separator(dt:pd.DataFrame, min_termination: float, min_duration:float = None, 
    ...
    storm_info["stormDuration"] = (storm_info["stormEndTime"] - storm_info["stormStartTime"]).apply(lambda x:x.total_seconds()/3600) # hours
